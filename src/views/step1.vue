<template>
  <div class="step1">
    <Input
      ...
      :rows="4"
      :style="{width:width+'%'}"
      placeholder="请输入NFV VIM北向接口ListResourcesMap接口返回的结果"
    />
    <Input
      v-model="vimcm"
      type="textarea"
  ...
  methods: {
    ...
    parseListResourceMap() {
      ...
      for (let i = 0; i < hosts.length; i++) {
        ...
        _host.cpu_unalloc_ratio =
          (hosts[i].vcpus - hosts[i].vcpus_used) / hosts[i].vcpus;
        ...
        _host.numa_node1_free_cpus =
          hosts[i].numa_topology[0].cpuset.length -
          hosts[i].numa_topology[0].cpu_usage;
        let node1Info = hosts[i].numa_topology[0].mempages[1];
        _host.numa_node1_total_mem =
          (node1Info.total * node1Info.size_kb) / 1024;
        _host.numa_node1_free_mem =
          ((node1Info.total - node1Info.used - node1Info.reserved) *
            node1Info.size_kb) /
          1024;
        ...
        _host.numa_node2_free_cpus =
          hosts[i].numa_topology[1].cpuset.length -
          hosts[i].numa_topology[1].cpu_usage;
        let node2Info = hosts[i].numa_topology[1].mempages[1];
        _host.numa_node2_total_mem =
          (node2Info.total * node2Info.size_kb) / 1024;
        _host.numa_node2_free_mem =
          ((node2Info.total - node2Info.used - node2Info.reserved) *
            node2Info.size_kb) /
          1024;
        _host.numa_node2_other = JSON.stringify(hosts[i].numa_topology[1]);
        _host.total_bandwidths_ingress =
          hosts[i].host_bandwidth_ingress_ovs_default_all_avalible_bandwidth -
          hosts[i].host_bandwidth_ingress_ovs_default_allocated_bandwidth;
        _host.total_bandwidths_egress =
          hosts[i].host_bandwidth_egress_ovs_default_all_avalible_bandwidth -
          hosts[i].host_bandwidth_egress_ovs_default_allocated_bandwidth;
    ...
    parseVimCM(tableData) {
      ...
      for (let i = 0; i < tableData.length; i++) {
        ...
        _host.cHAs = hostNameAndHost[_host.name].cHAs
          ? hostNameAndHost[_host.name].cHAs[0]
          : "";
        if (_host.cHAs != "") {
          let ha = {};
          ha.label = _host.cHAs;
          ha.value = _host.cHAs;
          if (
            this.allHAs.length == 0 ||
            JSON.stringify(this.allHAs).indexOf(JSON.stringify(ha)) == -1
          ) {
            this.allHAs.push(ha);
          }
        }
