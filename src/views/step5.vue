    ...
    <Tabs active-key="key1">
      ...
      <Tab-pane label="拓扑图" key="key3">
        ...
        <Select v-model="vnf" style="width:300px" @on-change="changeVnf">
          <Option v-for="item in vnfs" :key="item.label" :value="item.value">{{ item.label }}</Option>
        </Select>
        <div id="canvasDiv">
...
export default {
  ...
  data() {
    return {
      ...
      vnf: "",
      vmIdAndVm: {},
      vnfAndVmIds: this.$store.getters.getVNFAndVmIds,
      sourceHosts: "",
      hostColumns: [
        {
          type: "index",
        ...
        },
        {
          ...
          fixed: "left",
          render: (h, params) => {
            let vmIdList = params.row.vmIdList;
            if (this.vnf != "" && this.vnfAndVmIds[this.vnf] != undefined) {
              for (
                let i = 0, len = this.vnfAndVmIds[this.vnf].length;
                i < len;
                i++
              ) {
                let vmId = this.vnfAndVmIds[this.vnf][i];
                if (vmIdList.indexOf(vmId) > -1) {
                  vmIdList = vmIdList
                    .toString()
                    .replace(vmId, '<font color="red">' + vmId + "<font>");
                }
              }
            }
  ...
  computed: {
    currentVNF() {
      return this.$store.getters.getCurrentVNF;
    }
  },
  watch: {
    currentVNF: function(nVal, oVal) {
      this.vnf = nVal;
    }
  },
  created() {
  ...
  methods: {
      ...
      for (let i = 0, len = todoHosts.length; i < len; i++) {
        ...
        if (moveInIdList != undefined && moveInIdList.length > 0) {
          for (let i = 0, len = moveInIdList.length; i < len; i++) {
            ...
            after_cpuutil +=
              Math.round(
                ((vm.vCpus * vm.vcpuutil) / parseInt(host.total_cpu)) * 100
              ) / 100;
          }
        }
    ...
    doMove(hosts, hostNameAndVMs) {
      ...
      for (let i = 0, len = hosts.length; i < len; i++) {
        ...
        if (
          sourceHost.after_total_free_cpu == sourceHost.total_cpu ||
          sourceHost.after_total_free_cpu == 0
        ) {
          continue;
        }
        ...
        for (let j = 0, len = vms.length; j < len; j++) {
          ...
          for (let k = hosts.length - 1; k > 0; k--) {
            ...
            if (
              targetHost.after_total_free_cpu == targetHost.total_cpu ||
              targetHost.after_total_free_cpu == 0
            ) {
              continue;
            }
    ...
    allowedDeployVmToHost: function(vm, host) {
      if (
        host.after_numa_node1_free_cpus < vm.vCpus &&
        host.after_numa_node2_free_cpus < vm.vCpus
      ) {
        return false;
      }
      if (
        host.after_numa_node1_free_mem < vm.vRams &&
        host.after_numa_node2_free_mem < vm.vRams
      ) {
        return false;
      }
      if (
      ...
      if (
        vm.type != undefined &&
        vm.type == "anti_affinity" &&
        vm.name != undefined &&
        host.anti_affinity.index(vm.name) > -1
      ) {
        return false;
      }
      return true;
    ...
    computeAndRecordResource: function(vm, sourceHost, targetHost) {
      ...
      if (
        ...
        console.error(
          "CPU OR MEM is not enough,vm:{},targetHost:{}",
          vm,
          targetHost
        );
        return;
      ...
      targetHost.after_cpu_unalloc_ratio =
        targetHost.after_total_free_cpu / targetHost.total_cpu;
    ...
    changeVnf(data) {
      ...
      // canvas.height = cxt.height;
      Topo.clear();
      // let newCanas = document.createElement("canvas");
      // newCanas.id = "canvas";
      // newCanas.style = "background-color: black";
      // canvasDiv.appendChild(newCanas);
      this.drawTopo();
    },
    drawTopo() {
      let haAndHosts = this.haAndHosts;
      // let haAndHosts = topoTestData
      // this.vnf = this.vnfs[0].label;
      let canvas = document.getElementById("canvas");
      Topo.init(canvas);
      for (let ha in haAndHosts) {
        let haContainer = Topo.buildContainer(
          ha,
          "Top_Center",
          "130,178,210",
          10,
    ...
    addHostToSourceContainer(hosts) {
      ...
      let hostsContainer = Topo.buildContainer(
        "源主机",
        "Top_Center",
        "200,200,200",
        10,
      ...
      for (let i = 0, len = hosts.length; i < len; i++) {
        ...
        let hostContainer = Topo.buildContainer(
          hostName,
          "Bottom_Center",
          "10,10,100",
          this.haIndex * this.haDistance,
          sourceHostIndexY + this.hostYDistance,
          100,
          sourceHostHeight
        );
    ...
    addHostToTargetContainer(hosts) {
      ...
      for (let i = hosts.length - 1; i >= 0; i--) {
        ...
        let hostContainer = Topo.buildContainer(
          ...
          "Bottom_Center",
          "10,10,100",
          this.haIndex * this.haDistance + this.hostXDistance,
          targetHostIndexY + this.hostYDistance,
          100,
          targetHostHeight
        );
        let hostLayout = JTopo.layout.GridLayout(Math.round(vmNum / 2), 2);
        ...
        for (let j = 0; j < hosts[i].move_in_vmIdList.length; j++) {
          ...
          let vm = Topo.buildNode(
            vmId.substr(vmId.length - 3, vmId.length),
            "255,0,0"
          );
          vm.fullName = vmId;
