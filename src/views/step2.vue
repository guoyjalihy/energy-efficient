<template>
  <div class="step2">
    ...
    <Input
      ...
      :rows="4"
      :style="{width:width+'%'}"
      placeholder="请输入VM_PM接口返回的结果"
    />
    <Input
      v-model="queryVNFResource"
      type="textarea"
      :rows="4"
      :style="{width:width+'%'}"
      placeholder="请输入QueryVNFResource接口的返回结果"
    />
    <Button type="primary" @click="parse">解析</Button>
    <IntegrationTable
      :showVnf="true"
      :selectedVNFs="allVnfs"
      :columns="columns"
      :tableData="tableData"
    />
  </div>
...
export default {
  ...
  methods: {
    ...
    parseHA(tableData) {
      ...
      for (let i = 0, len = tableData.length; i < len; i++) {
        let vm = tableData[i];
        vm.cHAs = hostNameAndHost[vm.cHost]
          ? hostNameAndHost[vm.cHost].cHAs
          : "";
      }
    },
    parseVNFs(tableData) {
      ...
      for (let i = 0, len = tableData.length; i < len; i++) {
        ...
        if (vm.vnf_name != "") {
          ...
          if (
            this.allVnfs.length == 0 ||
            JSON.stringify(this.allVnfs).indexOf(JSON.stringify(vnfSelect)) ==
              -1
          ) {
            this.allVnfs.push(vnfSelect);
          }

          if (vnfAndVmIds[vm.vnf_name] == undefined) {
            vnfAndVmIds[vm.vnf_name] = [];
          }
          vnfAndVmIds[vm.vnf_name].push(vm.name);
        }
      }
      this.$store.commit("setAllVNFs", this.allVnfs);
      this.$store.commit("setVNFAndVmIds", vnfAndVmIds);
