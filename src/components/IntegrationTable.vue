<template>
  <div>
    <Divider />
    <Input v-model="name" placeholder="请输入名称（支持模糊查询）" style="width: 300px" clearable/>&nbsp
    <Select v-model="ha" :selectedHAs="selectedHAs" style="width:300px" placeholder="请选择HA" clearable>
      <Option v-for="item in has" :value="item.value" :key="item.value">{{ item.label }}</Option>
    </Select>&nbsp
    <Select v-model="vnf" v-show="showVnf" :selectedVNFs="selectedVNFs" style="width:300px" placeholder="请选择VNF" clearable>
      <Option v-for="item in vnfs" :value="item.value" :key="item.value">{{ item.label }}</Option>
    </Select>&nbsp
    <Button type="primary" @click="query">查询</Button>&nbsp
    <Button type="primary" @click="exportData()"><Icon type="ios-download-outline"></Icon>导出全部数据</Button>
...
<script>
  export default {
    ...
    methods: {
      query() {
        if(this.name == '' && (this.ha == '' || this.ha == undefined) && (this.vnf == '' || this.vnf == undefined)){
          this.tData = this.tableData.slice(0,10)
          this.total = this.tableData.length
          return
        }
        this.queryData = this.tableData
          // if((this.ha == '' || this.ha == undefined) && host.name.indexOf(this.name) > -1){
          //     this.queryData.push(host)
          // }else if(this.name == '' && host.cHAs.indexOf(this.ha) > -1){
          //     this.queryData.push(host)
          // }else if(host.name.indexOf(this.name) > -1 && host.cHAs.indexOf(this.ha) > -1){
          //     this.queryData.push(host)
          // }
        let ha = this.ha
      ...
      changePage (page) {
        ...
        if(this.name != '' || this.ha != '' || this.ha == undefined){
